...
class GlobalInnoCount(object):
    # Singleton tracker of innovation numbers
    ...
    def get_instance():
        ...
        if GlobalInnoCount.__instance == None:
            GlobalInnoCount()
        return GlobalInnoCount.__instance
    ...
    def __init__(self):
        ...
        if GlobalInnoCount.__instance != None:
            ...
            GlobalInnoCount.__instance = self
            self.count = 0
...
class Genome(object):
    ...
    def __init__(self, input_count, output_count):
        self.connections = []
        self.nodes = []
        self.innovation_counter = GlobalInnoCount.get_instance()

        # Generate input and output nodes
        for ii in range(input_count):
            self.nodes.append(
                Node(self.innovation_counter.get_next_count(), node_type=NodeType.INPUT))

        for ii in range(output_count):
            self.nodes.append(
                Node(self.innovation_counter.get_next_count(), node_type=NodeType.OUTPUT))
        ...
        inputs = list(filter(lambda node: node.get_type()
                      is NodeType.INPUT, self.nodes))
        random.shuffle(inputs)
        outputs = list(filter(lambda node: node.get_type()
                       is NodeType.OUTPUT, self.nodes))
        random.shuffle(outputs)
        while True:
            try:
                from_node = inputs.pop()
                to_node = outputs.pop()
                self.connections.append(Connection(from_node.get_innovation(
                ), to_node.get_innovation(), self.innovation_counter.get_next_count()))
            except IndexError:
    ...
    def add_connection(self):
        ...
        valid_nodes = list(filter(lambda node: node.get_type()
                           is not NodeType.OUTPUT, self.nodes))

        # Check that they're not the same layer
        got_connection = False
        new_innovation = self.innovation_counter.get_next_count()
        while not got_connection:
            ...
            if node_a.layer > node_b.layer:
                new_conn = Connection(node_b.get_innovation(
                ), node_a.get_innovation(), new_innovation)
            elif node_a.layer < node_b.layer:
                new_conn = Connection(node_a.get_innovation(
                ), node_b.get_innovation(), new_innovation)
            else:
                continue
            ...
            # Ok, passed both checks, so keep the connection
            if got_connection:
    ...
    def add_node(self):
        ...
        in_layer = [n for n in self.nodes if n.get_innovation() ==
                    conn.input][0].layer
        out_layer = [n for n in self.nodes if n.get_innovation() ==
                     conn.output][0].layer
        new_layer = (in_layer + out_layer)/2.0

        # Create a new node, random type, in the calculated layer
        new_node = Node(self.innovation_counter.get_next_count(), new_layer)
        self.nodes.append(new_node)

        # Connections for new node in place of old connection
        self.connections.append(Connection(
            conn.input, new_node.get_innovation(), self.innovation_counter.get_next_count()))
        self.connections.append(Connection(new_node.get_innovation(
        ), conn.output, self.innovation_counter.get_next_count()))

        # Random node of a lower layer than this one. Strictly lower, so we don't get loops.
        lower_node = random.choice(list(filter(
            lambda n: n.layer < new_layer and n.innovation != conn.input, self.nodes)))
        # TODO what if there is no lower node?
        self.connections.append(Connection(lower_node.get_innovation(
        ), new_node.get_innovation(), self.innovation_counter.get_next_count()))
...
def get_input_nodes(genome, node):
    ...
    nodes_out = list(filter(lambda conn: (
        conn.output == node.innovation) and conn.enabled, genome.connections))
...
def build_expression(genome, node):
    ...
    if node.get_type() is NodeType.OUTPUT:
        ...
        node_innov = list(filter(lambda conn: (
            conn.output == node.innovation) and conn.enabled, genome.connections))
        assert(len(node_innov) == 1)
        node_innov = node_innov[0].input
        # Get the node itself and make the recursive call
        out_node = list(filter(lambda node: node.innovation ==
                        node_innov, genome.nodes))[0]
        return f"out_{node.innovation} = {build_expression(genome, out_node)}"
    elif node.get_type() is NodeType.AND:
        expr = " AND  ".join([build_expression(genome, n)
                             for n in get_input_nodes(genome, node)])
        return f"({expr})"
    elif node.get_type() is NodeType.OR:
        expr = " OR ".join([build_expression(genome, n)
                           for n in get_input_nodes(genome, node)])
        return f"({expr})"
    elif node.get_type() is NodeType.NAND:
        expr = " NAND ".join([build_expression(genome, n)
                             for n in get_input_nodes(genome, node)])
        return f"({expr})"
    elif node.get_type() is NodeType.NOR:
        expr = " NOR ".join([build_expression(genome, n)
                            for n in get_input_nodes(genome, node)])
        return f"({expr})"
    ...
    with open(filename, 'w') as dotfile:
        ...
        for node in genome.nodes:
            if node.get_type() is NodeType.INPUT:
                dotfile.write(
                    f"  {node.get_innovation()} [shape=invtriangle]\n")
        ...
        for conn in genome.connections:
            if conn.enabled:
                ...
                dotfile.write(
                    f"  {conn.input} -> {conn.output} [style=dotted]\n")
        dotfile.write("}\n")
